...
按键盖内部有一个与弹片接触的横梁，其高度经过专门设计，过高或过低都会影响输入效果。一些非原装按键盖可能会出现这个问题。
...
除固定臂平面外，弹片还由另外三个平面组成，它们都属于触发臂。其中与感应器方向一致、用于遮挡红外信号的部分称为 **遮光片**。

当按键被按下时，遮光片会伸入感应器中，从而产生输入信号。

#### 1.2.1. 弹片的预变形

固定臂在自由状态下的变形状态称为弹片的 **预变形（pre-deformation）**，也就是弹片的“弧度”。
通常表现为向内或向外的偏移：
...
弹片预变形越向外偏移，按键通常越硬，回弹感越强；越向内偏移，按键越软，总行程越短。但值得指出的是，弹片的预变形并不决定按键相对于底座的触发位置。

新弹片通常没有偏移，接近平直状态。一般来说，并不需要对新弹片的预变形做明显调节。如需调节按键软硬和回弹力度，也应仅做极小幅度调整，使弧度微微可见即可。

如果将弹片过度地向内偏移，会严重影响按键的回弹性能和输入效果。请阅读 [第三节](#3-按键问题维修)。

对于某些材料比较脆弱的弹片，过度的弯折可能会显著减少其寿命。

#### 1.2.2. 弹片的折弯角

固定臂与触发臂相连的两个平面形成的 L 形二面角称为弹片的 **折弯角**。
...
折弯角过大时，遮光片伸入感应器的面积很小，不易形成稳定的遮光区域，因此“吃键”等问题的概率会显著提高。
...
不同的是，遮光片高度对遮光面积的影响不太大；但是遮光片高度的可调范围有限，且调节难度更大，对弹片的伤害也更大。因此，实际中常常以调节折弯角来代替调节遮光片高度。

对于材料较脆弱的弹片，用力调节遮光片高度可能会使其断裂。
...
调节折弯角，实际上也是在调节 **遮光片的有效长度**。因此，调节遮光片长度与调节折弯角的效果几乎相同。但是，遮光片长度的调节难度更大，且调节量不易控制。
...
弹片通过两个 *M3\*8* 规格的大扁头自攻螺丝固定在底座结构上，其固定状态不仅决定安装可靠性，也会影响弹片在受力过程中的实际工作形态。
